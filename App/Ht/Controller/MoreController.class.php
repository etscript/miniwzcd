...
class MoreController extends PublicController{
	...
	public function logindex() {
		$where="1=1";
		define('rows',20);
		$count=M('warn_log')->where($where)->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit = $page*rows;
		$page_index = $this->page_index($count,$rows,$page);
		$list = M('warn_log')->where($where)->order('warn_time desc,id desc')->limit($limit,rows)->select();
		foreach ($list as $k => $v) {
			$info = M('charge_log')->where('id='.intval($v['charge_id']))->find();
			$list[$k]['cplid'] = $info['cplid'];
			$list[$k]['warn_time'] = date("Y-m-d H:i:s",$v['warn_time']);
		}
	...
	public function logdel() {
		$id = intval($_GET['did']);
		$check_info = M('warn_log')->where('id='.intval($id))->find();
		if (!$check_info) {
		...
		//修改对应的删除状态
		$up = M('warn_log')->where('id='.intval($id))->delete();
		if ($up) {
